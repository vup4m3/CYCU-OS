...
public:
  ...
  int next_arrival() {
	if (queue_.size() != 0)
	  return queue_[0].d_arrival_time;
	else return -1;
  }
...
private:
  ...
  bool condition(int first, int second, int type) {
	...
	else if (type == kArrivalThanBigId) {
	  if (queue_[first].d_arrival_time < queue_[second].d_arrival_time || (queue_[first].d_arrival_time == queue_[second].d_arrival_time && queue_[first].d_id > queue_[second].d_id) )
		return true;
	  else return false;
	}
  ...
  bool preemptive(int type, int comparator) { // type == kSjf, comparator should be other remain_burst
	if (isBlocking) {
	  if (type == kSjf) {
		...
		if (currrent_process_.d_arrival_time < comparator)
		  return true;
		else return false;
	  }
	}
...
void lcfs(vector<Data> origin_data, TimeRecord &time_record) {
  Queue arrival_queue(origin_data, kArrivalThanId);
  Queue cpu_queue;
  Data current_process;
  CPU cpu(999);
  ...
  do
  {
	...
	else if (cpu.preemptive(klifo, cpu_queue.next_burst_remain())) { // preemptive
	  cpu_queue.push_back(cpu.get_process());
...
void nP(vector<Data> origin_data, TimeRecord &time_record) {
  Queue arrival_queue(origin_data, kArrivalThanPriority);
  Queue cpu_queue;
  Data current_process;
  CPU cpu(999);
